...
def test(model: Model, x, edge_index):
    ...
    split = get_split(num_samples=z.size()[0], train_ratio=0.9)
    evaluator = LREvaluator(num_epochs=5000) 
    result = evaluator.evaluate(z, data.y, split)
    return result
...
if __name__ == '__main__':
    ...
    print(f'aug_type={args.aug_type}, encoder={args.encoder}, std={args.std}')
