...
def loss_cl(x1, x2, tau=0.5, loss_type='info'):
    ...
    if loss_type == 'info':
        ...
        sim_matrix_a = torch.exp(sim_matrix_a / tau)
        pos_sim_a = sim_matrix_a[range(batch_size), range(batch_size)]
        loss_a = 1 / (sim_matrix_a.sum(dim=1) - pos_sim_a)
        loss_a = - torch.log(loss_a).mean()

        sim_matrix_b = torch.einsum('ik,jk->ij', x2, x1) / torch.einsum('i,j->ij', x2_abs, x1_abs)
        sim_matrix_b = torch.exp(sim_matrix_b / tau)
        pos_sim_b = sim_matrix_b[range(batch_size), range(batch_size)]
        loss_b = 1 / (sim_matrix_b.sum(dim=1) - pos_sim_b)
        loss_b = - torch.log(loss_b).mean()
        ...
        sim_matrix_a = torch.exp(sim_matrix_a / tau)
        pos_sim_a = sim_matrix_a[range(batch_size), range(batch_size)]
        loss_a = - torch.log(pos_sim_a).mean()

        sim_matrix_b = torch.einsum('ik,jk->ij', x2, x1) / torch.einsum('i,j->ij', x2_abs, x1_abs)
        sim_matrix_b = torch.exp(sim_matrix_b / tau)
        pos_sim_b = sim_matrix_b[range(batch_size), range(batch_size)]
        loss_b = - torch.log(pos_sim_b).mean()
...
def train_cl_with_sim_loss(view_gen1, view_gen2, view_optimizer, model, optimizer, data_loader, device, tau, loss_type):
    loss_all = 0
    model.train()
    total_graphs = 0
    for data in data_loader:
        ...
        cl_loss = loss_cl(out1, out2, tau=tau, loss_type=loss_type)
...
def cl_exp(args):
    ...
    lr = args.lr
    dataset = get_dataset(args.dataset, sparse=True, feat_str='deg+odeg100', root='datasets')
    ...
    if epochs == 0:
        best_test_acc, best_test_std = eval_acc(model, data_eval_loader, device)
        
    for epoch in range(1, epochs+1):
        train_loss = train_cl_with_sim_loss(view_gen1, view_gen2, view_optimizer, model, optimizer, data_loader, device, tau=args.tau, loss_type=args.loss)
    ...
    # best_test_acc = test_acc = eval_acc(model, data_eval_loader, device)
    ...
    #    f.write('{},{},{:.2f}\n'.format(args.dataset, args.seed, best_test_acc*100))
    return best_test_acc, best_test_std
...
class simclr_graph_cl(nn.Module):
    ...
    def loss_cal(self, x, x_aug, tau=0.2, loss_type='info'):
        batch_size, _ = x.size()
...
if __name__ == '__main__':
    args = arg_parse()
    print(f'dataset={args.dataset}, loss={args.loss}, hidden_dim={args.hidden_dim}')
    ...
    if args.exp == 'cl_exp':
        ...
        graph_cl_exp(args)
    # print(f'(E): Acc = {np.mean(accs): .4f} +- {np.std(accs): .4f}')
    print()
