...
class Encoder(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int, activation, base_model=GCNConv, k: int = 2, skip=False, norm=None):
...
class GRACE(torch.nn.Module):
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor, epoch, loss_type):
        if loss_type == 'info':
        ...
        if loss_type == 'uniform':
            ...
            refl_sim = f(refl_sim)
            between_sim = f(between_sim)
            return -torch.log(1 / (between_sim.sum(1) + refl_sim.sum(1) - refl_sim.diag()))
        raise NotImplementedError
    

    def semi_loss_bmm(self, z1: torch.Tensor, z2: torch.Tensor, epoch, args, bmm_model, fit = False, loss_type = 'info'):
        f = lambda x: torch.exp(x / self.tau)
        refl_sim = self.sim(z1, z1)
        between_sim = self.sim(z1, z2)
        ...
        if args.mode == 'weight':
            ...
            if loss_type == 'uniform':
                ...
                ng_bet = (between_sim.masked_select(mask).view(N,-1) * B).sum(1) / B.mean(1)
                ng_refl = (refl_sim.masked_select(mask).view(N,-1) * B).sum(1) / B.mean(1)
                return -torch.log(1/(between_sim.diag() + ng_bet + ng_refl)) 
            raise NotImplementedError
            ...
            if loss_type == 'uniform':
                ...
                between_sim = f(between_sim)
                M = f(M)
                return -torch.log(1 / (M.sum(1) + between_sim.sum(1) + refl_sim.sum(1) - refl_sim.diag()))  
            raise NotImplementedError
        else:
            print('Mode Error!')
            raise NotImplementedError
    ...
    def loss(self, z1: torch.Tensor, z2: torch.Tensor, epoch, args, bmm_model,mean: bool = True, batch_size: Optional[int] = None, loss_type: str = None):
        
        h1 = self.projection(z1)
        h2 = self.projection(z2)
        if epoch < args.epoch_start:
            if batch_size is None:
                l1 = self.semi_loss(h1, h2, epoch, loss_type)
                l2 = self.semi_loss(h2, h1, epoch, loss_type)
            else:
            ...
            if batch_size is None:
                l1 = self.semi_loss_bmm(h1, h2, epoch, args, bmm_model, fit = True, loss_type = loss_type)
                l2 = self.semi_loss_bmm(h2, h1, epoch, args, bmm_model, fit = True, loss_type = loss_type)
            else:
