...
from pGRACE.model import Encoder, GRACE, BetaMixture1D, NormLayer
from pGRACE.functional import drop_feature, drop_edge_weighted, degree_drop_weights, evc_drop_weights, pr_drop_weights, \
    feature_drop_weights, drop_feature_weighted_2, feature_drop_weights_dense
from pGRACE.eval import log_regression, MulticlassEvaluator, LREvaluator
from pGRACE.utils import get_base_model, get_activation, generate_split, compute_pr, eigenvector_centrality
...
def train(epoch, bmm_model, args):
    model.train()
    optimizer.zero_grad()
    ...
    loss = model.loss(z1, z2, epoch, args, bmm_model, batch_size=512 if args.dataset == 'Physics' else None, loss_type=args.loss)
    loss.backward()
    optimizer.step()
    return loss.item()
...
if __name__ == '__main__':
    ...
    print(f'dataset={args.dataset}, lr={learning_rate}, tau={tau}, epochs={num_epochs}, activation={args.act}, loss={args.loss}, drop_scheme={drop_scheme}\n'
          f'num_hidden={num_hidden}, num_proj_hidden={num_proj_hidden}, pe1={drop_edge_rate_1}, pe2={drop_edge_rate_2}, pf1={drop_feature_rate_1}, pf2={drop_feature_rate_2} '
          f'weight_init={args.weight_init}, iters={args.iters}, epoch_start={args.epoch_start}')
    ...
    with tqdm(total=num_epochs, desc='(T)') as pbar:
        for epoch in range(1, num_epochs + 1):
            loss = train(epoch, bmm_model, args)
