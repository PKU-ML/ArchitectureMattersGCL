...
class TUDataset_aug(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None,
                 ...
                 cleaned=False, aug=None, aug_std=None, ratio=None):
        ...
        if not (self.name in ['MUTAG', 'PTC_MR', 'DD', 'PROTEINS', 'NCI1', 'NCI109']):
    ...
    def get(self, idx):
        ...
        if self.aug == 'dnodes':
            data_aug = drop_nodes(deepcopy(data), self.ratio)
        elif self.aug == 'pedges':
            data_aug = permute_edges(deepcopy(data), self.ratio)
        elif self.aug == 'subgraph':
            data_aug = subgraph(deepcopy(data), self.ratio)
        elif self.aug == 'mfeatures':
            data_aug = mask_features(deepcopy(data), self.ratio)
        elif self.aug == 'dfeatures':
            data_aug = dropout_features(deepcopy(data), self.ratio)
        elif self.aug == 'none':
            data_aug = deepcopy(data)
        elif self.aug == 'noise':
            data_aug = noise_nodes(data, self.aug_std)

        elif self.aug == 'random2':
            n = np.random.randint(2)
            if n == 0:
               data_aug = permute_edges(deepcopy(data)) # drop_nodes(deepcopy(data))
            elif n == 1:
            ...
            if n == 0:
               ...
               data_aug = mask_features(deepcopy(data))
            else:
                print('sample error')
                assert False
...
def noise_nodes(data, std):
    data.x =  data.x + std ** 0.5 * torch.rand(data.x.size())
    return data
...
def dropout_features(data, ratio):
    data.x = F.dropout(data.x, p=ratio)
    return data
